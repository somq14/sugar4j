...
final class ThreeStepMethod {
  ...
  ThreeStepMethod(Map<String, String> options) {
  ...
  private void generateObjective2() throws Exception {
    ...
    for (int weight : softConstraints.keySet()) {
      ...
      for (Constraint constraint : softConstraints.get(weight)) {
        if (constraint.getName().startsWith("C14_")) {
          continue;
        }
        sugar4j.addConstraints(translator.translate(constraint));
  ...
  private void generateObjective3() throws Exception {
    int maxWeight = softConstraints.lastKey();
    ...
    for (int weight : softConstraints.keySet()) {
      ...
      for (Constraint constraint : softConstraints.get(weight)) {
        if (constraint.getName().startsWith("C14_")) {
          sugar4j.addConstraints(translator.translate(constraint));
          terms.add(create(Expression.MUL, create(constraint.getWeight()),
              translator.getPenaltyVariableOf(constraint)));
          penaltyUpperBound += constraint.getWeight() * constraint.getPenaltyUpperBound();
        }
      }
    }

    Expression obj3 = create("_P3");

    Expression varExp =
        create(Expression.INT_DEFINITION, obj3, Expression.ZERO, create(penaltyUpperBound));
    sugar4j.addExpression(varExp);
    log("Add Constraint %s", varExp.toString());

    Expression consExp = create(Expression.EQ, obj3, create(Expression.ADD, terms));
    sugar4j.addConstraint(consExp);
    log("Add Constraint %s", consExp.toString());
  ...
  private void generateObjective4(int ans1, int ans2, int ans3) throws Exception {
    Expression obj4 = create("_P4");

    int maxWeight = softConstraints.lastKey();
    Expression varExp = create(
        Expression.INT_DEFINITION, obj4, Expression.ZERO, create(maxWeight * ans1 + ans2 + ans3));
    sugar4j.addExpression(varExp);
    log("Add Variable %s", varExp);

    Expression consExp = create(Expression.EQ, obj4,
        create(Expression.ADD, create(Expression.MUL, create(maxWeight), create("_P1")),
            create("_P2"), create("_P3")));
    sugar4j.addExpression(consExp);
    log("Add Constraint %s", varExp);
  ...
  private void searchSolution() throws Exception {
    ...
    Expression obj3 = Expression.create("_P3");
    int ans3 = solution.getIntMap().get(obj3);
    log("Found OBJ = %d", ans3);

    while (true) {
      log("Search OBJ <= %d", ans3 - 1);

      sugar4j.addAssumption(obj3, Comparator.LE, ans3 - 1);
      solution = sugar4j.solve(timeout);
      solveCount++;
      ...
      ans3 = solution.getIntMap().get(obj3);
      sugar4j.addConstraint(create(Expression.LE, obj3, create(ans3)));
      log("Found OBJ = %d", ans3);
      bestSolution = solution;
    }

    Expression obj3Bind = create(Expression.LE, obj3, create(ans3));
    log("Add Constraint: %s", obj3Bind.toString());
    sugar4j.addConstraint(obj3Bind);
    log("Done");

    log("-------------------------------- 4th STEP --------------------------------");

    generateObjective4(ans1, ans2, ans3);
    solution = sugar4j.solve();
    solveCount++;
    ...
    Expression obj4 = Expression.create("_P4");
    int ans4 = solution.getIntMap().get(obj4);
    log("Found OBJ = %d", ans4);

    while (true) {
      log("Search OBJ <= %d", ans4 - 1);

      sugar4j.addAssumption(obj4, Comparator.LE, ans4 - 1);
      solution = sugar4j.solve(timeout);
      solveCount++;
      ...
      ans4 = solution.getIntMap().get(obj4);
      sugar4j.addConstraint(create(Expression.LE, obj4, create(ans4)));
      log("Found OBJ = %d", ans4);
      bestSolution = solution;
    }
